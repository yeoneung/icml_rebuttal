...
- Q2: 
- Removing (Ouyang et al., 2019)'s restrictive assumption lead to the expotential growth of the state norm, causing the cost to increase exponentially, both theoretically and experimentally, resulting in a steep increase in regret. To address this issue, we initially switched to the rejection method using the compact set employed in Abeille & Lazaric(2018). The concentration of the true system parameter is $\tilde O(1/\sqrt{\lambda_{\min}})$ and as it converges, the probability of drawing values closer to the true system parameter increases, reducing the probability of sampling system parameter that cause the state to explode. Therefore, by adding noise to action, we induced polynomial growth of $\lambda_{\min}$, minimizing the probability of drawing system parameters causing the state to explode, which in turn led to the derivation of a constant bound for state moments. 
...
Thank you so much for carefully reading our paper and giving us invaluable input. Your comments are all to the point and we would like to address them very carefully. Regardless of this, the paper you mentioned, 'Single timescale actor-critic method to solve the linear quadratic regulator with convergence guarantees', the authors implement the inverse of the Fisher information matrix for more efficient gradient descent and convergence guarantee which aligns with the core idea of our paper yet scope is a bit different as we focus on improved regret. We will make sure to include in the related work in the revised version. Followings are answers to your comments.
...
- Q2: Similar to Q1, Proposition 4.1 holds only if samples from exact and approximate posterior share the same Brownian motion. On the other hand, two key properties for the exact posterior $\mu_t$ stated in Proposition 4.2 and the first part of Theorem 4.5 hold true regardless of the coupling. To be more specific, we only use the martingale properties for them. In the last part of the proof of the second part (concentration property of $\tilde \mu_t$)in Theorem 4.5 (line 1986 - 1795), we use the argument like $\mathbb{E}[|\tilde \theta_t|]
