...
- Q2: 
  - Concerning Assumption 2.1, we would kindly refer to answer to Q2 raised by 'Reviewer is77' due to the space constraint.
...
- Q4: 
  - Here, $a_n = O(b_n)$ means $\limsup_{n \to \infty} |a_n/b_n |< \infty$, employ $a_n = \Omega(b_n)$ for $\liminf_{n \to \infty}|a_n/b_n |>0$. We also admit that $N \geq O( (\lambda_{\max} / \lambda_{\min})^2)$ should be changed to $N = \Omega( (\lambda_{\max} / \lambda_{\min})^2)$.  
